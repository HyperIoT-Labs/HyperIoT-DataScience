package it.acsoftware.hyperiot.spark.visitrate
...
object VisitRate {
  ...
  def concatenateRowsToJson(df: DataFrame, hPacketFieldId: Long): String = {
    ...
    val rows = df.collect().map(row => {
      ...
      val output = row.getAs[Double]("visitPercentage")
  ...
  def main(args: Array[String]) = {
    ...
    */
    ...
    val visitId = root.input.each.mappedInputList.each.packetFieldId.long.getAll(jobConfig).drop(2).headOption.get
      ...
      try {
          ...
          df.transform { df =>
            ...
            .select(
              ...
              when(col("hPacketField.id") === visitId, coalesce(                                                                             // coalesce
                col("hPacketField.value.member0").cast("string"), 
                col("hPacketField.value.member1").cast("string"),
                col("hPacketField.value.member2").cast("string"), 
                col("hPacketField.value.member3").cast("string"),
                col("hPacketField.value.member4").cast("string"), 
                col("hPacketField.value.member5").cast("string"))).as("visitId")
            )
          }
    ...
    // Normal flow
      ...
      val visitDF = values.select(col("visitId").cast("string"))

      // Remove null values
      val nonNullValueDF = valueDF.na.drop()
      val nonNullExbDF = exbDF.na.drop()
      val nonNullVisitDF = visitDF.na.drop()
      ...
      val df3WithIndex = nonNullVisitDF.withColumn("index", monotonically_increasing_id())
      ...
      // Compute output
      val totalSessionsPerExh = resultDF
        .select("exhibitionId", "visitId")
        .distinct()
        .groupBy("exhibitionId")
        .agg(countDistinct("visitId").alias("totalSessions"))

      val objectVisitCounts = resultDF
        .select("exhibitionId", "value", "visitId")
        .distinct() // only 1 visit per visitId
        .groupBy("exhibitionId", "value")
        .agg(countDistinct("visitId").alias("sessionCount"))

      val percentages = objectVisitCounts
        .join(totalSessionsPerExh, Seq("exhibitionId"))
        .withColumn(
          "visitPercentage",
          col("sessionCount") / col("totalSessions")
        )
        .withColumn("timestamp", current_timestamp().cast("long"))

        val resDF = percentages.select("value", "exhibitionId", "visitPercentage", "timestamp")
