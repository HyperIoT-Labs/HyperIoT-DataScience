package it.acsoftware.hyperiot.spark.minrangecountby
...
object MinRangeCountBy {
  ...
  def concatenateRowsToJson(df: DataFrame, hPacketFieldId: Long): String = {
    ...
    val rows = df.collect().map(row => {
      ...
      val exhibitionId  = row.getAs[String]("exhibitionId")

      val less_than_1_min = row.getAs[Long]("less_than_1_min")
      val between_1_and_3_min = row.getAs[Long]("between_1_and_3_min")
      val more_than_3_min = row.getAs[Long]("more_than_3_min")

      Map("grouping" -> Map(hPacketFieldId -> groupingValue, "exhibitionId" -> exhibitionId), 
          "output" -> "",
          "outputArray" -> Map( "less_than_1_min" -> less_than_1_min, 
                           "between_1_and_3_min" -> between_1_and_3_min,
                           "more_than_3_min" -> more_than_3_min) 
      )
    })
  ...
  def main(args: Array[String]) = {
    ...
    */
    ...
    // Get the hpacketFieldId of ExhibitionId, startDate and endDate
    val exhibitionId = root.input.each.mappedInputList.each.packetFieldId.long.getAll(jobConfig).drop(1).headOption.get
    val startDateFieldId = root.input.each.mappedInputList.each.packetFieldId.long.getAll(jobConfig).drop(2).headOption.get
    val endDateFieldId = root.input.each.mappedInputList.each.packetFieldId.long.getAll(jobConfig).drop(3).headOption.get
      ...
      try {
          ...
          df.transform { df =>
            ...
            .filter(
              ...
              col("hPacketField.id") === exhibitionId ||
            ...
            .select(
              ...
              when(col("hPacketField.id") === exhibitionId, coalesce(                                                                             // coalesce
                col("hPacketField.value.member0").cast("string"), 
                col("hPacketField.value.member1").cast("string"),
                col("hPacketField.value.member2").cast("string"), 
                col("hPacketField.value.member3").cast("string"),
                col("hPacketField.value.member4").cast("string"), 
                col("hPacketField.value.member5").cast("string"))).as("exhibitionId"),
    ...
    */
    ...
    // normal flow
      ...
      val rowCountExbNA = nonNullExbDF.count()
      ...
      if (rowCountVNA == rowCountSNA && rowCountSNA == rowCountENA) {      
        ...
        val df2WithIndex = nonNullExbDF.withColumn("index", monotonically_increasing_id())
        val df3WithIndex = nonNullStartDateDF.withColumn("index", monotonically_increasing_id())
        val df4WithIndex = nonNullEndDateDF.withColumn("index", monotonically_increasing_id())

        // Merge dataframe using this fake index
        val resultDF = df1WithIndex
          .join(df2WithIndex, Seq("index"))
          .join(df3WithIndex, Seq("index"))
          .join(df4WithIndex, Seq("index"))
        ...
        val dfWithRange = dfWithDuration.withColumn(
          "duration_range",
          when(col("duration") < firstRange, "less_than_1_min")
            .when(col("duration") < secondRange, "between_1_and_3_min")
            .otherwise("more_than_3_min")
        )

        val countByRange = dfWithRange
          .groupBy("value", "exhibitionId", "duration_range")
          .agg(count("*").as("count"))

        val resDF = countByRange
          .groupBy("value", "exhibitionId")
          .pivot("duration_range", Seq("less_than_1_min", "between_1_and_3_min", "more_than_3_min"))
          .agg(first("count"))
        ...
        // Retrieve timestamp
        val timestampValue = resDF.select("timestamp").first().getLong(0)
        ...
        // Create JSON file
        val jsonData = concatenateRowsToJson(resDF, hPacketFieldId)
